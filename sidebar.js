// sidebar.js
const renderSidebar = () => {
    // 1. Remove qualquer resquício de sidebar anterior
    const existingSidebar = document.querySelector('aside');
    if (existingSidebar) existingSidebar.remove();

    // 2. Cria o HTML da Sidebar
    const sidebarHTML = `
    <aside class="w-64 bg-white border-r border-gray-100 flex flex-col fixed h-full z- font-sans left-0 top-0">
        ...
        <div class="p-4 border-t border-gray-50">
            <a href="perfil.html" class="flex items-center gap-3 p-3 rounded-2xl bg-[#f8fafc] border border-gray-100 hover:border-blue-200 hover:bg-blue-50 transition-all group shadow-sm no-underline">
                ...
                <i class="fas fa-chevron-right text-gray-300 group-hover:text-blue-500 text-[10px] transition-colors"></i>
            </a>
        </div>
    </aside>`;

    // 3. Insere a sidebar no início do body
    document.body.insertAdjacentHTML('afterbegin', sidebarHTML);

    // 4. Carrega os dados salvos do Dr. Victor
    ...
    if (dados.nome && nomeEl) nomeEl.innerText = dados.nome;
    if (dados.categoria && tagEl) tagEl.innerText = dados.categoria;
    if (avatarEl) {
        if (dados.foto) {
            avatarEl.innerHTML = `<img src="${dados.foto}" class="w-full h-full object-cover">`;
        } else if (dados.nome) {
            avatarEl.innerText = dados.nome.charAt(0).toUpperCase();
        }
    }
};
